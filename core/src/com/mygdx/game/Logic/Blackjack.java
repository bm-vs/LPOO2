...
public class Blackjack
{
    ...
    public Card giveCard(int Id) // Id=0 isDealer
    {
        ...
        if (players.get(Id).getValuePlayer() == 21 && players.get(Id).aceExists() && players.get(Id).getcartasJogada().size() == 2) {
            players.get(Id).blackjack = true;
        }
    ...
    public void win() {
        ...
        if (valueDealer > 21) {
            ...
            if (players.get(PLAYER).blackjack && !players.get(DEALER).blackjack) {
                WIN = true;
                P.setMoney(P.getMoney() + 2.5 * aposta);
                P.addEarningsBlackjack(2.5 * aposta);
            }
            else if (!players.get(PLAYER).blackjack && players.get(DEALER).blackjack) {
                LOSE = true;
            }
